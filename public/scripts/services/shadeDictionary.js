angular.module('ShadeApp')
    ...
    .service('ShadeIdentifiers', function (ShadeStaticHandlers, ShadeHandlers) {

        var sh = ShadeStaticHandlers,
            sw = ShadeHandlers;

        return (function (key, an, av, sn, sv, c, cb, mn) {
            return {
                'attrNames' : {type: "Attribute Name", keys: key(an || {})},
                'attrValues' : {type: "Attribute Value", keys: key(av || {})},
                'styleNames' : {type: "Style Name", keys: key(sn || {})},
                'styleValues' : {type: "Style Value", keys: key(sv || {})},
                'controls': {type: "Control", keys: key(c || {})},
                'controlBlocks': {type: "Control Block", keys: key(cb || {})},
                'mainNodes': {type: "", keys: key(mn || {})}
            };
