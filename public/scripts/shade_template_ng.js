...
var gstyles = "", //styles will be appended to this variable as they are created, to be later appended to the HTML ouput of this template
    ...
    //TODO: have dictionaries as helper objects in the template
    ...
    handleAttributes = function (value, attr) {
        %> <%=attrNameHandlers[attr] || attr%> = "<%=attrValueHandlers[value] || value%>" <%
    },

    handleStyles = function (styles, value, style) {
        return styles += (style && value) ? (styleNameHandlers[style] || style) + ':' + (styleValueHandlers[value]  || value) + '; ' : '';
    },
    ...
    openElement = function (elmName, className, node, customStyles) {
        ...
        %><<%=elmName%> <%
        if (className || customStyles || nativeStyles) {
            %>class = "<%=className%> class<%=classCount%>"<%
        } 
        _.each(node, handleAttributes)%>><% 
        if (customStyles || nativeStyles) {
            addStyles("class" + (++classCount - 1), (customStyles || '') + (nativeStyles || ''));
        }
    },
